...
function OutputQualityFilterForm() {
  ...
  const { actions: dashboardActions } = useDashboardOutputQuality()
  ...
  const handleChangeOrder = (id) => {
    ...
    switch (qcStageId) {
      case STAGE_OPTION.SO_EXPORT:
        dashboardActions.getQcCheckItemBySo({ soId: id }, (data) => {
          setItemList(data)
        })
        break
  ...
  const handleSubmit = () => {
    ...
    dashboardActions.getOutputQualityDashboard(params)
  }
