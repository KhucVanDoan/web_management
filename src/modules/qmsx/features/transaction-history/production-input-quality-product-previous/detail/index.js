...
const breadcrumbs = [
  ...
  {
    route:
      ROUTE.TRANSACTION_HISTORY.PRODUCTION_INPUT_QUALITY_PRODUCT_PREVIOUS_DETAIL
        .PATH,
    title:
      ROUTE.TRANSACTION_HISTORY.PRODUCTION_INPUT_QUALITY_PRODUCT_PREVIOUS_DETAIL
        .TITLE,
...
function ProductionInputQualityProductPreviousDetail() {
  ...
  const workCenter = isArray(workCenters) ? workCenters[0]?.name : ''
  const formattedNumberOfTime =
    numberOfTimeQc && numberOfTime ? `${numberOfTimeQc}/${numberOfTime}` : ''
  ...
  const backToList = () => {
    const location = {
      ...
      search: '?tab=qcProductionInputProductPrevious',
    }
  ...
  useEffect(() => {
    actions.getDetailProductionInputQualityProductPreviousTransactionHistory(
      { id: id },
      null,
      null,
    )
...
export default ProductionInputQualityProductPreviousDetail
