...
const InputQualityFilterForm = () => {
  ...
  const handleChangeStage = (id) => {
    ...
    if (
      parsedQcStageId !== STAGE_OPTION.PO_IMPORT &&
      parsedQcStageId !== STAGE_OPTION.PRO_IMPORT
    )
      return
  ...
  const handleChangeOrder = (code) => {
    ...
    if (parsedQcStageId === STAGE_OPTION.PO_IMPORT)
      actions.getItemListByPo({ poId: id }, (data) => {
        setItemList(data)
      })
    else if (parsedQcStageId === STAGE_OPTION.PRO_IMPORT)
