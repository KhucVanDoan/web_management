...
import { defineQualityAlertInputSchema } from './schema'

const ENDPOINT_PATCH_GET_ORDER_BY_STAGE_INPUT = 'env-input-order'
...
const ENDPOINT_PATCH_GET_WAREHOUSE_BY_ORDER_ID_AND_PRODUCT_ID = {
  PO: 'env-warehouse-by-po-and-item',
  PRO: 'env-warehouse-by-pro-and-item',
  IMO: 'env-warehouse-by-imo-and-item',
}

function DefineQualityAlertInputForm() {
  ...
  const MODE_MAP = {
    [ROUTE.DEFINE_QUALITY_ALERT.CREATE_INPUT.PATH]: MODAL_MODE.CREATE,
    [ROUTE.DEFINE_QUALITY_ALERT.EDIT_INPUT.PATH]: MODAL_MODE.UPDATE,
  }
  ...
  const getBreadcrumb = () => {
    ...
    switch (mode) {
      case MODAL_MODE.CREATE:
        breadcrumbs.push({
          route: ROUTE.DEFINE_QUALITY_ALERT.CREATE_INPUT.PATH,
          title: ROUTE.DEFINE_QUALITY_ALERT.CREATE_INPUT.TITLE,
        })
        break
      case MODAL_MODE.UPDATE:
        breadcrumbs.push({
          route: ROUTE.DEFINE_QUALITY_ALERT.EDIT_INPUT.PATH,
          title: ROUTE.DEFINE_QUALITY_ALERT.EDIT_INPUT.TITLE,
        })
        break
      default:
        break
  ...
  const getTitle = () => {
    switch (mode) {
      case MODAL_MODE.CREATE:
        return ROUTE.DEFINE_QUALITY_ALERT.CREATE_INPUT.TITLE
      case MODAL_MODE.UPDATE:
        return ROUTE.DEFINE_QUALITY_ALERT.EDIT_INPUT.TITLE
      default:
        break
    }
  }
  ...
  const initialValues = {
    ...
    purchasedOrderId: qualityAlertDetail?.purchasedOrder?.id,
    itemId: qualityAlertDetail?.item?.id,
    warehouseId: qualityAlertDetail?.warehouse?.id,
    errorReportId: qualityAlertDetail?.errorReport?.id,
    description: qualityAlertDetail?.description || '',
    vendorName: qualityAlertDetail?.purchasedOrder?.vendor || '',
  }
  ...
  const getOrder = (stageQcValue) => {
    const params = {
      id: stageQcValue,
      endpointPatch: ENDPOINT_PATCH_GET_ORDER_BY_STAGE_INPUT,
    }
  ...
  const getWarehouse = (stageQcValue, orderId, productId) => {
    ...
    switch (+stageQcValue) {
      ...
      case STAGE_OPTION.IMO_IMPORT:
        endpointPatch =
          ENDPOINT_PATCH_GET_WAREHOUSE_BY_ORDER_ID_AND_PRODUCT_ID.IMO
        break
      default:
        break
    }
      ...
      <Grid container justifyContent="center">
        <Grid item xl={11} xs={12}>
          <Formik
            ...
            validationSchema={defineQualityAlertInputSchema(t)}
            onSubmit={onSubmit}
            enableReinitialize
            onChange
          >
            ...
            {({ handleReset, setFieldValue, values }) => (
              <Form>
                ...
                >
                  ...
                  {values?.stage === STAGE_OPTION.PO_IMPORT &&
                    values?.purchasedOrderId && (
                      <Grid item lg={6} xs={12}>
                        <Field.TextField
                          name="vendorName"
...
export default DefineQualityAlertInputForm
