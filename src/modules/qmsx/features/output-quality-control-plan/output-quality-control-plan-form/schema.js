...
export const OutputQualityControlPlanSchema = (t, mode, status) =>
  Yup.object().shape({
    code: Yup.string()
            ...
            return schema.test({
              ...
              test: () => {
                if (
                  ...
                  (status === OUTPUT_QC_PLAN_STATUS_OPTIONS.CONFIRMED ||
                    status === OUTPUT_QC_PLAN_STATUS_OPTIONS.INPROGRESS) &&
                  context.value < 0
                ) {
                  return false
                }
