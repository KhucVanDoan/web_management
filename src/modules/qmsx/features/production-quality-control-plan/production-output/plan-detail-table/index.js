...
const PlanDetailTable = (props) => {
  const { items, mode, setFieldValue } = props
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (mode === MODAL_MODE.DETAIL) {
          ...
          return (
            <Field.Autocomplete
              name={`form[${keyBomTree}-${id}].userQc1st`}
              multiple
              disabled={!isQc}
              ...
              onChange={() => {
                setFieldValue(`form[${keyBomTree}-${id}].bomId`, bomId)
                setFieldValue(`form[${keyBomTree}-${id}].type`, type)
                setFieldValue(`form[${keyBomTree}-${id}].routingId`, routingId)
                setFieldValue(
                  `form[${keyBomTree}-${id}].keyBomTree`,
                  keyBomTree,
                )
              }}
            />
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (mode === MODAL_MODE.DETAIL) {
          ...
          return (
            <Field.Autocomplete
              name={`form[${keyBomTree}-${id}].userQc2nd`}
              multiple
        ...
        if (mode === MODAL_MODE.DETAIL) {
          ...
          return (
            <Field.DateRangePicker
              name={`form[${keyBomTree}-${id}].productionQcPlanDate`}
              disabled={!isQc}
              minDate={new Date(workOrders[0]?.planFrom)}
              onChange={() => {
                setFieldValue(`form[${keyBomTree}-${id}].itemId`, itemId)
                setFieldValue(
                  `form[${keyBomTree}-${id}].workOrderId`,
                  workOrders[0]?.id,
                )
                setFieldValue(
                  `form[${keyBomTree}-${id}].qualityPointId`,
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (mode === MODAL_MODE.DETAIL) {
          ...
          return (
            <Field.TextField
              name={`form[${keyBomTree}-${id}].planErrorRate`}
              disabled={!isQc}
              type="number"
              InputProps={{
                endAdornment: (
              ...
              onChange={(value) => {
                setFieldValue(
                  `form[${keyBomTree}-${id}].producingStepId`,
                  producingStep?.id,
                )

                setFieldValue(
                  `form[${keyBomTree}-${id}].planningQuantity`,
                  planningQuantity,
                )
                setFieldValue(
                  `form[${keyBomTree}-${id}].actualQuantityBom`,
                  actualQuantityBom,
                )
                setFieldValue(
                  `form[${keyBomTree}-${id}].actualQuantityProducingStep`,
                  actualQuantityProducingStep,
                )
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        if (mode === MODAL_MODE.DETAIL) {
          ...
          return (
            <Field.TextField
              name={`form[${keyBomTree}-${id}].planQcQuantity`}
              disabled={!isQc}
              type="number"
            />
          )
  ...
  const onChangePlanErrorRate = (value, objectValue, setFieldValue) => {
    if (value !== '') {
      ...
      setFieldValue(
        `form[${objectValue?.keyBomTree}-${objectValue?.id}].planQcQuantity`,
        Math.ceil(cal),
      )
    } else {
      setFieldValue(
