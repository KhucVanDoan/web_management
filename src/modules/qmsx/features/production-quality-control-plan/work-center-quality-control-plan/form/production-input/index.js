...
const WorkCenterQualityControlPlanProductionInputForm = () => {
  ...
  const MODE_MAP = {
    [ROUTE.WORK_CENTER_QUALITY_CONTROL_PLAN.EDIT_INPUT.PATH]: MODAL_MODE.UPDATE,
  }
  ...
  useEffect(() => {
    if (mode === MODAL_MODE.UPDATE) {
      const params = {
        ...
        endpointPatch:
          ENDPOINT_PATCH_UPDATE_WORK_CENTER_QC_PLAN.INPUT_SCHEDULES,
      }
  ...
  const breadcrumbs = [
    ...
    {
      route: ROUTE.WORK_CENTER_QUALITY_CONTROL_PLAN.EDIT_INPUT.PATH,
      title: ROUTE.WORK_CENTER_QUALITY_CONTROL_PLAN.EDIT_INPUT.TITLE,
    },
  ]
    ...
    if (wcQcPlanDetail) {
      wcQcPlanDetail?.workInShiftQcPlan?.dayInShift?.forEach((e, idx) => {
        columns.push({
          ...
          width: 130,
          renderCell: (params) => {
            const { keyRow, indexOfMaterial } = params?.row
            if (keyRow === KEY_ROW_TABLE.PLAN_QUANTITY) {
              return (
                <Field.TextField
                  name={`workInShiftQcPlan.dayInShift[${idx}].scheduleShiftDetails[${indexOfMaterial}].planQuantity`}
                  type="number"
  ...
  const getRowQC = (values) => {
    ...
    values?.workInShiftQcPlan?.dayInShift?.forEach((e) => {
      if (maxLength === e?.scheduleShiftDetails?.length) {
        title = e?.scheduleShiftDetails.map((v) => v?.itemMaterialName)
        indexOfMaterial = e?.scheduleShiftDetails.map((_, index) => index)
      }
    })
    for (let i = 0; i < maxLength; i++) {
      let shiftTemp = [
      ...
      planingAmount['title'] = title[i]
      planingAmount['indexOfMaterial'] = indexOfMaterial[i]
      matrix = matrix.concat([
        [3, 1, 1],
        [-1, 1, 1],
        [-1, 1, 1],
  ...
  const onSubmit = (values) => {
    const paramsUpdate = {
      type: TYPE_WC_QC_PLAN.INPUT_MATERIAL,
...
export default WorkCenterQualityControlPlanProductionInputForm
