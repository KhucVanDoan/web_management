...
function DefineShelf() {
  ...
  const columns = [
    ...
    {
      field: 'description',
      headerName: t('defineShelf.description'),
      width: 120,
    },
    {
      field: 'status',
      headerName: t('defineShelf.status'),
      width: 120,
      renderCell: (params) => {
        ...
        return (
          <div>
            <IconButton
              onClick={() =>
                history.push(
                  ROUTE.DEFINE_SHELF.DETAIL.PATH.replace(':id', `${id}`),
                )
              }
            >
              <Icon name="show" />
            ...
            <IconButton
              onClick={() =>
                history.push(
                  ROUTE.DEFINE_SHELF.EDIT.PATH.replace(':id', `${id}`),
                )
              }
            >
              <Icon name="edit" />
  ...
  const onSubmitUpdateStatus = () => {
    if (modal.tempItem?.status === ACTIVE_STATUS.ACTIVE) {
      actions.rejectShelfById(modal.tempItem?.id, () => refreshData())
    } else if (modal.tempItem?.status === ACTIVE_STATUS.INACTIVE) {
      actions.confirmShelfById(modal.tempItem?.id, () => {
        refreshData()
      })
    }
  ...
  const renderHeaderRight = () => {
    return (
      <>
        <ImportExport
          onImport={(params) => importShelfApi(params)}
          onExport={() =>
            exportShelfApi({
        ...
        />
        <Button
          onClick={() => history.push(ROUTE.DEFINE_SHELF.CREATE.PATH)}
          sx={{ ml: 4 / 3 }}
          icon="add"
        >
