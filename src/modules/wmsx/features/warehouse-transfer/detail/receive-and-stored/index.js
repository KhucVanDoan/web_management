...
const ReceiveAndStored = () => {
  ...
  const breadcrumbs = [
    ...
    {
      route: ROUTE.WAREHOUSE_TRANSFER.RECEIVE.PATH,
    ...
    () => ({
      items: warehouseTransferDetails?.warehouseTransferDetailLots?.map(
        (item, index) => ({
          ...
          actualExportedQuantity: +item?.planQuantity || 0,
          inputedQuantity: +item?.planQuantity || 0,
  ...
  const onSubmit = (values) => {
    const params = {
      items: values?.items?.map((item) => ({
        ...
        quantity: +item?.inputedQuantity,
      })),
    }
    actions.confirmWarehouseImportById(params, backToList)
  }
  return (
    <Page
      breadcrumbs={breadcrumbs}
      title={t('menu.receiveAndStored')}
      onBack={backToList}
      loading={isLoading}
    >
      <Formik
...
export default ReceiveAndStored
